...
export const NotificationModal: React.FC<NotificationModalProps> = ({
  ...
  const [emailList, setEmailList] = useState<string[]>(() => {
    ...
    return list.length ? list : ['thanhtgndt@gmail.com', 'kythuat@tasago.vn'];
  });
  ...
  const [emailSender, setEmailSender] = useState(config.emailSender || 'Bê Tông Tasago');
  ...
  const inputClass = 'w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500';
  const resultBox = (result: ActionResult) => result && <div className={`mt-3 rounded-lg border px-3 py-2 text-sm ${result.success ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-800'}`}><span className="inline-flex items-center gap-1 font-medium">{result.success ? <CheckCircle2 className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}{result.message}</span></div>;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-2 sm:p-5" onClick={onClose}>
      <div className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-slate-50 shadow-2xl" onClick={event => event.stopPropagation()}>
        <div className="flex items-center justify-between bg-emerald-800 px-4 py-3 text-white sm:px-6"><div><h2 className="text-base font-bold sm:text-lg">Trung tâm thông báo Email</h2><p className="text-xs text-emerald-100">Gửi tự động qua Gmail HTTPS relay, không dùng cổng SMTP</p></div><button onClick={onClose} className="rounded-lg p-2 hover:bg-emerald-700" aria-label="Đóng"><X className="w-5 h-5" /></button></div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
          {activeTab === 'email' && <div className="space-y-5">
            ...
            <section className="rounded-xl border border-blue-200 bg-blue-50 p-4"><h3 className="font-bold text-blue-900">Gmail HTTPS relay</h3><p className="mt-1 text-xs leading-relaxed text-blue-800">Gmail được gửi qua một Google Apps Script HTTPS relay. Cách này tránh hoàn toàn lỗi timeout do Render Free chặn các cổng SMTP 25, 465 và 587. Secret relay chỉ nằm trên server Render/Vercel.</p><label className="mt-3 block text-xs font-semibold text-slate-700">Tên hiển thị người gửi<input value={emailSender} onChange={event => setEmailSender(event.target.value)} className={inputClass} /></label><div className="mt-4 flex flex-wrap gap-2"><button onClick={handleVerifyRelay} disabled={verifyingRelay} className="inline-flex items-center gap-1 rounded-lg border border-blue-300 bg-white px-3 py-2 text-xs font-bold text-blue-800 hover:bg-blue-100 disabled:opacity-60"><Server className="w-4 h-4" />{verifyingRelay ? 'Đang kiểm tra...' : 'Kiểm tra Gmail relay'}</button><button onClick={handleSaveConfig} className="inline-flex items-center gap-1 rounded-lg bg-emerald-700 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-800"><Check className="w-4 h-4" />Lưu cấu hình</button></div>{resultBox(verifyResult)}{resultBox(sendResult)}</section>
          </div>}
